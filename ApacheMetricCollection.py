...
class ApacheMetricCollection:
    ...
    def clone_and_process_gh_repos(self, repos: list) -> None:
        ...
        for repo in repos:
            ...
            print(f"Cloning the project {repo[0]}...")
            process.wait()
            ...
            if (process.poll() == 0):
                ...
                num_revisions = self.get_project_revision_count(repo[0])
                num_authors = self.get_project_author_count(repo[0])
                first_revision_date = self.get_first_commit_date(repo[0])
                self.write_data_to_csv(repo[0], num_revisions, num_authors, first_revision_date)
            elif(process.poll() == 128):
                # clone unsuccessful -- print error and continue
                print(f"Repository for project {repo[0]} was not found on GitHub")
                self.write_data_to_csv(repo[0], 0, 0, 0)

            print(f"Collected metrics for {repo[0]}; removing the directory ./repositories/{repo[0]}...\n")
            self.remove_repo_directory(repo[0])
    ...
    def write_data_to_csv(self, repo_name: str, num_revisions: int, num_authors: str, first_revision_date: str) -> None:
        with open('csv/ApacheProjectMetrics.csv', 'a') as file:
            writer = csv.writer(file, csv.QUOTE_NONNUMERIC)
            writer.writerow([repo_name, num_revisions, num_authors, first_revision_date])
    
    def create_csv_file(self) -> None:
        with open('csv/ApacheProjectMetrics.csv', 'w') as file:
            writer = csv.writer(file, csv.QUOTE_NONNUMERIC)
            writer.writerow(['Project Name', 'Number of Revisions', 'Number of Authors', 'Date of First Commit'])
